...
# Identify empty cells in original puzzles
empty_cells_mask = (X.reshape(-1, 81, 9).sum(axis=2) == 0).astype(int)

# Split the data
train_size = 90000
X_train = torch.tensor(X[:train_size], dtype=torch.float32)
Y_train = torch.tensor(Y[:train_size], dtype=torch.long)
mask_train = torch.tensor(empty_cells_mask[:train_size], dtype=torch.float32)

X_val = torch.tensor(X[train_size:], dtype=torch.float32)
Y_val = torch.tensor(Y[train_size:], dtype=torch.long)
mask_val = torch.tensor(empty_cells_mask[train_size:], dtype=torch.float32)
...
def train_model(model, criterion, optimizer, train_loader, val_loader, epochs=10):
    ...
    for epoch in tqdm(range(epochs), desc="Training Progress"):
        ...
        for X_batch, Y_batch, mask_batch in train_loader:
            ...
            loss = criterion(outputs.view(-1, 9), Y_batch.view(-1)) * mask_batch.view(-1)
            loss = loss.sum() / mask_batch.sum()  # Average only over masked entries
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for X_batch, Y_batch, mask_batch in val_loader:
                ...
                val_loss = criterion(val_outputs.view(-1, 9), Y_batch.view(-1)) * mask_batch.view(-1)
                val_loss = val_loss.sum() / mask_batch.sum()
...
def evaluate_model(model, val_loader):
    ...
    with torch.no_grad():
        for X_batch, Y_batch, mask_batch in val_loader:
            ...
            correct += ((predicted_classes.view(-1) == Y_batch.view(-1)) * mask_batch.view(-1)).sum().item()
            total += mask_batch.sum().item()
